...
<main class="flex-1 p-6 bg-[#f4f6f9] overflow-y-auto min-h-screen">
    <div class="mb-5">
        <h2 class="text-lg font-bold text-gray-800 m-0">Tambah Kategori Pemohon</h2>
        <p class="text-xs text-gray-500 mt-0.5">Tambahkan kategori pemohon baru ke dalam sistem.</p>
    </div>

    <div class="bg-white border border-gray-200 rounded shadow-sm overflow-hidden max-w-2xl">
        <form action="{{ route('admin.kategori-pemohon.store') }}" method="POST" class="p-6">
            @csrf
            <div class="mb-5">
                ...
                <input type="text" name="nama_kategori" id="nama_kategori" value="{{ old('nama_kategori') }}" class="w-full text-sm px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors" required placeholder="Contoh: Perorangan, Lembaga Swadaya Masyarakat">
                @error('nama_kategori')
