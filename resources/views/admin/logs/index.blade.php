    ...
    <div class="bg-white border border-slate-200/80 rounded-2xl shadow-xs overflow-hidden">
        <div class="w-full overflow-hidden">
            ...
            <table class="w-full table-fixed text-left border-collapse text-xs">
                <thead>
                    <tr class="bg-slate-50/70 text-slate-500 text-[10px] uppercase tracking-wider border-b border-slate-100">
                        <th class="w-[15%] px-4 py-3 font-bold">Waktu</th>
                        <th class="w-[20%] px-4 py-3 font-bold">Aktor / Pengguna</th>
                        <th class="w-[18%] px-4 py-3 font-bold">No. Registrasi</th>
                        <th class="w-[20%] px-4 py-3 font-bold">Tahapan / Aksi</th>
                        <th class="w-[27%] px-4 py-3 font-bold">Catatan Detail</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 text-slate-600">
                    ...
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        <td class="px-4 py-3">
                            <span class="font-bold text-slate-800 block text-[11px] whitespace-nowrap">{{ $log->created_at->format('d M Y, H:i') }} WIB</span>
                            <span class="text-[10px] text-slate-400 font-mono block">{{ $log->created_at->diffForHumans() }}</span>
                        </td>
                        <td class="px-4 py-3">
                            <div class="flex items-center gap-2">
                                <div class="w-6 h-6 rounded-full bg-blue-100 text-blue-700 flex items-center justify-center font-bold text-[11px] shrink-0">
                                    {{ substr($log->user->name ?? 'S', 0, 1) }}
                                </div>
                                <div class="min-w-0">
                                    <span class="font-bold text-slate-800 block text-xs truncate">{{ $log->user->name ?? 'Sistem Otomatis' }}</span>
                                    <span class="text-[10px] text-slate-400 block truncate">{{ $log->user->role->name ?? 'System' }}</span>
                                </div>
                            </div>
                        </td>
                        <td class="px-4 py-3">
                            @if($log->permohonan_informasi)
                            <a href="{{ route('admin.permohonan.show', $log->permohonan_informasi->id) }}" class="inline-flex items-center gap-1 font-mono font-bold text-blue-600 hover:underline text-xs truncate">
                            ...
                            <span class="block text-[10px] text-slate-400 truncate">{{ $log->permohonan_informasi->nama_pemohon }}</span>
                            @else
                            <span class="text-slate-400 italic text-[11px]">-</span>
                            @endif
                        </td>
                        <td class="px-4 py-3">
                            <span class="inline-flex items-center gap-1 px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider bg-blue-50 text-blue-700 border border-blue-200/80 shadow-2xs">
                                <span class="material-symbols-outlined text-[12px]">bolt</span>
                                {{ $log->aksi }}
                            </span>
                            @if($log->tahapan_proses)
                            <span class="block text-[9px] text-slate-400 mt-0.5 uppercase font-semibold tracking-wider truncate">Tahap: {{ $log->tahapan_proses }}</span>
                            @endif
                        </td>
                        <td class="px-4 py-3 text-slate-700 text-xs">
                            @if($log->catatan)
                            <div class="p-2 bg-slate-50 border border-slate-200/60 rounded-lg text-[11px] leading-snug italic text-slate-600 line-clamp-2" title="{{ $log->catatan }}">
                                "{{ $log->catatan }}"
                            </div>
                            @else
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-4 py-12 text-center text-slate-400">Belum ada riwayat alur permohonan yang sesuai filter pencarian.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
            ...
            <table class="w-full table-fixed text-left border-collapse text-xs">
                <thead>
                    <tr class="bg-slate-50/70 text-slate-500 text-[10px] uppercase tracking-wider border-b border-slate-100">
                        <th class="w-[16%] px-4 py-3 font-bold">Waktu (WIB)</th>
                        <th class="w-[22%] px-4 py-3 font-bold">Pengguna</th>
                        <th class="w-[18%] px-4 py-3 font-bold">Aksi Operasi</th>
                        <th class="w-[30%] px-4 py-3 font-bold">Deskripsi Perubahan</th>
                        <th class="w-[14%] px-4 py-3 font-bold">Alamat IP</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-slate-100 text-slate-600">
                    @forelse($logs as $log)
                    <tr class="hover:bg-slate-50/80 transition-colors">
                        ...
                        <td class="px-4 py-3">
                            @if($log->user)
                            <div class="flex items-center gap-2">
                                <div class="w-6 h-6 rounded-full bg-slate-200 text-slate-700 flex items-center justify-center font-bold text-[11px] shrink-0">
                                    {{ substr($log->user->name, 0, 1) }}
                                </div>
                                <div class="min-w-0">
                                    <span class="font-bold text-slate-800 block text-xs truncate">{{ $log->user->name }}</span>
                                    <span class="text-[10px] text-slate-400 font-mono block truncate">{{ $log->user->email }}</span>
                                </div>
                            </div>
                            @else
                        ...
                        <td class="px-4 py-3 whitespace-nowrap">
                            <span class="inline-flex items-center px-2 py-0.5 rounded-md text-[9px] font-bold uppercase tracking-wider bg-slate-100 text-slate-700 border border-slate-200 shadow-2xs">
                                {{ $log->action }}
                            </span>
                        </td>
                        <td class="px-4 py-3 text-slate-700 text-xs leading-relaxed truncate" title="{{ $log->description }}">
                            {{ $log->description ?? '-' }}
                        </td>
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="px-4 py-12 text-center text-slate-400">Tidak ada log keamanan/sistem yang tercatat.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
