...
@section('title', 'Edit Slide Banner - Admin E-PPID')
...
<main class="flex-1 p-5 md:p-8 bg-[#f8fafc] overflow-y-auto min-h-screen" x-data="{
    ...
    order: {{ old('order', $carousel->order) }},
    isActive: {{ old('is_active', $carousel->is_active) ? 'true' : 'false' }},
    imagePreview: '{{ $carousel->image_url }}',
    handleImageChange(e) {
    ...
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <div>
            <div class="flex items-center gap-2.5 mb-1">
                ...
                <h1 class="text-xl font-bold text-slate-900 m-0 tracking-tight">Edit Slide Banner</h1>
            </div>
            <p class="text-xs text-slate-500 ml-10.5">Perbarui teks, berkas visual, atau tautan slide banner carousel</p>
        </div>
            ...
            <div class="lg:col-span-7 space-y-6">
                ...
                <div class="bg-white rounded-2xl p-5 sm:p-6 border border-slate-200/80 shadow-xs space-y-4">
                    ...
                    <div>
                        <label class="block text-xs font-bold text-slate-700 mb-1">
                            Ganti Gambar Banner <span class="text-slate-400 font-normal">(Biarkan kosong jika tidak ingin mengubah)</span>
                        </label>
                            ...
                            <div class="space-y-2 text-center">
                                ...
                                <div class="flex text-xs text-slate-600 justify-center">
                                    <label for="image" class="relative cursor-pointer bg-white rounded-lg px-3 py-1 font-bold text-blue-600 hover:text-blue-700 border border-slate-200 shadow-2xs hover:border-blue-200 focus-within:outline-none">
                                        ...
                                        <input id="image" name="image" type="file" accept="image/jpeg,image/png,image/webp,image/svg+xml" class="sr-only" @change="handleImageChange($event)">
                                    </label>
                                </div>
                ...
                <div class="flex items-center justify-end gap-3 pt-2">
                    ...
                    <button type="submit" class="px-6 py-2.5 rounded-xl bg-blue-600 hover:bg-blue-700 text-white text-xs font-bold transition-all shadow-xs flex items-center gap-1.5 cursor-pointer">
                        ...
                        <span>Perbarui Slide Banner</span>
                    </button>
                </div>
            </div>
