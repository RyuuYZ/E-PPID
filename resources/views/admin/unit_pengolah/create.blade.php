...
<main class="flex-1 p-6 bg-[#f4f6f9] overflow-y-auto min-h-screen">
    <div class="mb-5">
        <div class="flex items-center gap-2 mb-1">
            ...
            <h2 class="text-lg font-bold text-gray-800 m-0">Tambah Bidang Baru</h2>
        </div>
        <p class="text-[11px] text-gray-500 ml-7">Tambahkan Unit Pengolah Data baru ke dalam sistem.</p>
    </div>
    ...
    <div class="max-w-2xl bg-white border border-gray-200 rounded shadow-sm p-6">
        <form action="{{ route('admin.unit-pengolah.store') }}" method="POST">
            ...
            <div class="mb-4">
                ...
                <input type="text" id="nama_bidang" name="nama_bidang" value="{{ old('nama_bidang') }}" required class="w-full text-sm px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors">
                @error('nama_bidang')
            ...
            <div class="mb-5">
                ...
                <textarea id="deskripsi" name="deskripsi" rows="4" class="w-full text-sm px-3 py-2 border border-gray-300 rounded focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500 transition-colors">{{ old('deskripsi') }}</textarea>
                @error('deskripsi')
                    <p class="mt-1 text-[11px] text-red-600 font-medium">{{ $message }}</p>
                @enderror
            </div>
